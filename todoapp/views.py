...
def loginuser(request):
    if request.method == "GET":
       ...
       if user is None:
           return render(request,'todoapp/loginuser.html', {'form': AuthenticationForm(), 'error': 'username and password didnot match'})
       else:
           login(request, user)
           return redirect('currenttodos')
    ...
    if request.method == "GET":
      ...
      try:
        ...
        return redirect('currenttodos')
          ...
          return render(request, 'todoapp/createtodo.html', {'form': TodoForm(), 'error':'bad data is passed'})
